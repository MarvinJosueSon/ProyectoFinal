...
class DocentesAD(ttk.Frame):
    ...
    def eliminar_docente(self):
        ...
        nombre = vals[1]
        usuario = vals[2]
        contrasena = vals[3]  # oculto en UI sería mejor
        id_huella = vals[4]

        if not messagebox.askyesno("Confirmación", f"¿Eliminar el docente '{codigo} - {nombre}' de la base de datos?"):
            return
